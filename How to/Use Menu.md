---
url: /camomilla-core/How to/Use Menu.md
---
# 🍜 Use Menu
...
If no template\_path is specified, the default template will be used.
...
`request` is available in the menu template via the standard request context processor (the menu renderer always binds it, falling back to `None`), so {{ item|node\_url:request }} is always safe to write — static links ignore the request and pass through verbatim.
