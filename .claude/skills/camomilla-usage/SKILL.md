...
Projects upgrading from camomilla ≤ 6.4 have data in the old `status` (translatable CharField PUB/DRF/PLA/TRS) + `publication_date` columns; the new system derives state from `published_at` + `deleted_at`. **Recommended path:** run `python manage.py camomilla_makemigrations` (a drop-in `makemigrations` wrapper, no app arg — covers camomilla's models AND custom `AbstractPage` subclasses in your own apps) — it detects the transition and auto-inserts the data step into each generated migration, so you just `migrate`, no hand-editing. Under the hood it injects the custom operation `camomilla.upgrades.MigrateStatusToLifecycle()` (also droppable into a plain `makemigrations` output by hand, or as a `RunPython(camomilla.upgrades.migrate_status_to_lifecycle, …)` callable). The transform maps every page model per-language (PUB→past published_at; PLA→publication_date; DRF/TRS→null; global `deleted_at` only when all languages are TRS), preserving the old `is_public` result exactly. Drafts start empty (the old system had no draft storage). Full procedure: the "Upgrading from status-based publication" docs page.
