...
from example.website.models import ExposedRelatedPageModel, UnexposedRelatedPageModel, RelatedPageModel
...
class PageRelationApi(TestCase):
    ...
    def test_exposed_relation(self):
        ...
        response = client.get("/api/camomilla/pages-router/exposed-related-page-model-1")
        assert response.status_code == 200
        data = response.json()
        assert "related_pages" in data, "Exposed related pages should be included in the API response"
    ...
    def test_unexposed_relation(self):
        ...
        response = client.get("/api/camomilla/pages-router/unexposed-related-page-model-1")
        assert response.status_code == 200
        data = response.json()
        assert "related_pages" not in data, "Unexposed related pages should not be included in the API response"